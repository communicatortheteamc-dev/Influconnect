...
const paymentTypeOptions = ["neft", "imps", "upi"];
...
export default function CampaignDetails() {
    ...
    const exportExcel = () => {
        const rows = getOrderedCampaignRows(filteredAddedInfluencers).map((row: any, index: number) => {
            ...
            if (exportFields.includes("paymentType")) result["Payment Type"] = row.paymentType || ""
            if (exportFields.includes("paymentNumber")) result["Payment Number"] = row.paymentNumber || ""
            if (exportFields.includes("uploadedDate")) result["Uploaded Date"] = row.uploadedDate || ""
            if (exportFields.includes("bankDetails")) result["Bank Details"] = row.bankDetails || ""
            if (exportFields.includes("upiId")) result["UPI ID"] = row.upiId || ""
