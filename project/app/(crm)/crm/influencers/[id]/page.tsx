...
export default function InfluencerProfile() {
  ...
  useEffect(() => {
    fetch(`/api/crm/influencers/${id}`)
      ...
      .then(res => {
        ...
        // Ensure phone and email exist in state
        influencer.phone = influencer.phone || ""
        influencer.email = influencer.email || ""
        influencer.budget = influencer.budget || ""
        influencer.negotiableBudget = influencer.negotiableBudget || ""
        setForm(influencer)
      })
  }, [id])
  ...
  const saveProfile = async () => {
    await fetch(`/api/crm/influencers/${id}`, {
      method: "PUT",
  ...
  const markAsComplete = async () => {
    ...
    const res = await fetch(`/api/crm/influencers/complete`, {
      ...
      body: JSON.stringify({
        ...form,
        _id: id,
        verifiedBy: user?.name || "Unknown" // person who clicked verify
      })
    })
  ...
  const categoryArray = Array.isArray(form?.category)
    ? form?.category
    : form?.category
      ? [form?.category]
      : [];
                  ...
                  {editing ? (
                    categories.map(cat => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-xs border ${categoryArray.includes(cat)
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 rounded-xl shadow-lg text-white">
            ...
            <p className="text-sm opacity-90 mb-6">
              Clicking this button moves the influencer to the <b>Full Profiles</b> collection. Only do this if all fields and social links are accurate.
            </p>
            <button
              onClick={markAsComplete}
              disabled={!isProfileComplete}
              className={`w-full font-bold py-3 rounded-lg transition active:scale-95
    ${isProfileComplete
  ...
  `}
          ...
          <div className="mt-8 border rounded-lg p-4 bg-white">
            ...
            {form?.editHistory && form.editHistory.length > 0 ? (
